/**
 * Browser-safe ATProto helpers. No Next.js `next:` cache options, no server
 * imports. All fetch calls use the public Bluesky AppView or PDS directly and
 * support CORS.
 */

import { DEFAULT_LANG, type LanguageGroup } from "@/lib/atproto/records";

export interface ClientActorProfile {
  did: string;
  handle: string;
...
export async function fetchActorProfile(
  ...
  try {
    ...
    const p = (await res.json()) as AppViewProfileResponse;
    return {
      did: p.did,
      handle: p.handle,
