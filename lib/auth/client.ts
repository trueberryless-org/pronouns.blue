...
function getPublicMetadata(): PublicClientMetadata {
  const base = process.env.PUBLIC_URL ?? "http://127.0.0.1:3000";
  return {
...
function getKeyset(): ClientAssertionPrivateJwk[] | undefined {
  ...
  if (publicUrl && privateKey) {
    return [JSON.parse(privateKey) as ClientAssertionPrivateJwk];
  }
  return undefined;
}
