...
export function SearchModal() {
  ...
  function closeModal() { setOpen(false); setQuery(""); setSuggestions([]); }
  ...
  return (
    <>
      <button type="button" onClick={() => setOpen(true)} aria-label="Search handles (Ctrl+K)" className="flex h-10 items-center gap-2 rounded-full border border-[var(--border)] bg-[var(--surface)] px-3 text-[var(--muted)] hover:border-[var(--accent)] hover:text-[var(--text)] transition-colors">
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 pt-[15vh] backdrop-blur-sm" onClick={(e) => { if (e.target === e.currentTarget) closeModal(); }} role="dialog" aria-modal="true" aria-label="Search">
          <div className="mx-4 w-full max-w-lg overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--surface)] shadow-2xl">
            <div className="flex items-center gap-3 border-b border-[var(--border)] px-4 py-3">
              <SearchIcon />
              <input ref={inputRef} value={query} onChange={(e) => setQuery(e.target.value.replace(/^@/, ""))} onKeyDown={(e) => { if (e.key === "Enter" && normalizedQuery) openHandle(normalizedQuery); if (e.key === "Escape") closeModal(); }} placeholder="Search handle…" className="flex-1 bg-transparent text-base text-[var(--text)] placeholder-[var(--muted)] outline-none" />
              <kbd className="inline-flex h-6 items-center rounded border border-[var(--border)] px-1.5 font-mono text-xs text-[var(--muted)]">esc</kbd>
            </div>
            ...
            {(loading || suggestions.length > 0 || normalizedQuery) && (
              <div className="max-h-72 overflow-auto p-2">
                {loading ? (
                  ...
                  <p className="px-3 py-2 text-sm text-[var(--muted)]">No results for &ldquo;{normalizedQuery}&rdquo;</p>
                ) : (
                  <ul>
                    {suggestions.map((s) => (
                      <li key={s.did}>
