...
export function AuthNav({ signedIn, did, handle, displayName, avatar }: AuthNavProps) {
  ...
  if (signedIn) {
    ...
    return (
      <div className="relative" ref={menuRef}>
        ...
        >
          {avatar ? (
            <span className="h-full w-full bg-cover bg-center" style={{ backgroundImage: `url(${avatar})` }} role="img" aria-label={label} />
          ) : (
        ...
        {open && (
          <div role="menu" className="absolute right-0 z-20 mt-2 min-w-44 rounded-lg border border-[var(--border)] bg-[var(--surface)] p-1 shadow-md">
            <a href={profileHref} role="menuitem" className="flex items-center gap-2 rounded-md px-3 py-2 text-sm text-[var(--text)] hover:bg-[var(--surface-strong)]" onClick={() => setOpen(false)}>
              <UserIcon /> Profile
            </a>
            <a href="/settings" role="menuitem" className="flex items-center gap-2 rounded-md px-3 py-2 text-sm text-[var(--text)] hover:bg-[var(--surface-strong)]" onClick={() => setOpen(false)}>
              <CogsIcon /> Settings
            </a>
            <button type="button" role="menuitem" onClick={handleLogout} className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-sm text-[var(--text)] hover:bg-[var(--surface-strong)]">
              <LogoutIcon /> Log out
            </button>
          </div>
        )}
    ...
    <div className="flex flex-col items-end gap-1">
      <div className="flex items-center gap-2">
        <input
          ...
          onKeyDown={(e) => { if (e.key === "Enter" && loginHandle.trim() && !loading) startLogin(); }}
