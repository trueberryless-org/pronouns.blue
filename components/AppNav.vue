...
onMounted(() => {
  watchEffect(async () => {
    ...
    try {
      const r = await fetch(`https://public.api.bsky.app/xrpc/app.bsky.actor.getProfile?actor=${currentDid}`);
      const p = await r.json();
      user.value = { did: currentDid, handle: p.handle, displayName: p.displayName ?? null, avatar: p.avatar ?? null };
    } catch {
      user.value = null;
    }
  });
...
const label = computed(() => user.value?.displayName ?? user.value?.handle ?? user.value?.did ?? 'profile');
const normalizedHandle = computed(() => user.value?.handle?.replace(/^@/, ''));
const profileHref = computed(() => normalizedHandle.value ? `/profile/${encodeURIComponent(normalizedHandle.value)}` : '/settings');
    ...
    <div class="mx-auto flex w-full max-w-6xl min-h-14 items-center gap-3 px-4 sm:px-6">
      ...
      <div class="ml-auto flex items-center gap-4">
        ...
        <div class="hidden items-center sm:flex">
          <!-- Loading skeleton -->
          <div v-if="user === undefined" class="h-10 w-10 animate-pulse rounded-full bg-[var(--surface-strong)]" />

          <!-- Signed in: avatar + dropdown -->
          <div v-else-if="signedIn" class="relative" ref="desktopMenuRef">
            <button
              type="button"
            ...
            >
              <span v-if="user?.avatar" class="h-full w-full bg-cover bg-center" :style="{ backgroundImage: `url(${user.avatar})` }" role="img" :aria-label="label" />
          ...
          <div v-else class="flex flex-col items-end gap-1">
            <div class="flex items-center gap-2">
              <input
                v-model="loginHandle"
                @keydown.enter="loginHandle.trim() && !loginLoading && startLogin()"
            ...
            <p v-if="loginError" class="text-xs text-[var(--danger)]">{{ loginError }}</p>
          </div>
        </div>
        ...
        <div class="relative sm:hidden" ref="mobileMenuRef">
          ...
          >
            <svg v-if="mobileMenuOpen" viewBox="0 0 24 24" class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
            <svg v-else viewBox="0 0 24 24" class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true"><line x1="3" y1="6" x2="21" y2="6" /><line x1="3" y1="12" x2="21" y2="12" /><line x1="3" y1="18" x2="21" y2="18" /></svg>
          </button>

          <div v-if="mobileMenuOpen" role="menu" class="absolute right-0 z-20 mt-2 min-w-56 rounded-lg border border-[var(--border)] bg-[var(--surface)] p-2 shadow-lg">
            <div class="border-b border-[var(--border)] px-1 pb-3 mb-2">
              <p class="mb-1.5 px-2 text-xs font-medium text-[var(--muted)]">Theme</p>
              <ThemeToggle />
            </div>

            <template v-if="signedIn">
              <div v-if="user?.avatar" class="flex items-center gap-2 px-3 py-2 mb-1">
