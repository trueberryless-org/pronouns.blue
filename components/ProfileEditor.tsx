...
function TutorialTooltip({ text, onDismiss }: { text: string; onDismiss: () => void }) {
  return (
    <div className="absolute bottom-full left-1/2 z-20 mb-2 -translate-x-1/2">
      <div className="animate-[fade-in-up_0.2s_ease-out_both]">
        <div className="flex items-center gap-2 whitespace-nowrap rounded-full bg-[var(--accent)] py-1 pl-3 pr-1.5 text-xs font-medium text-[var(--accent-contrast)] shadow-lg">
          {text}
          <button type="button" onClick={(e) => { e.stopPropagation(); onDismiss(); }} aria-label="Dismiss hint" className="flex h-4 w-4 items-center justify-center rounded-full bg-[var(--accent-contrast)]/20 opacity-80 hover:opacity-100">
            <svg viewBox="0 0 24 24" className="h-2.5 w-2.5" fill="none" stroke="currentColor" strokeWidth={3} strokeLinecap="round" aria-hidden="true">
              <line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" />
            </svg>
          </button>
        </div>
        <div className="absolute left-1/2 top-full -translate-x-1/2" style={{ width: 0, height: 0, borderLeft: "5px solid transparent", borderRight: "5px solid transparent", borderTop: "5px solid var(--accent)" }} />
      </div>
    </div>
  );
}
...
function EntryList({
  label, items, preferred, tutorialStep = 0,
  onTogglePreferred, onMoveUp, onMoveDown, onRemove,
  onTutorialAdvance, onTutorialDismiss,
}: {
  label: string; items: string[]; preferred: string[]; tutorialStep?: 0 | 1 | 2;
  onTogglePreferred: (item: string) => void;
  onMoveUp: (item: string) => void; onMoveDown: (item: string) => void;
  onRemove: (item: string) => void;
  onTutorialAdvance?: () => void; onTutorialDismiss?: () => void;
}) {
  if (items.length === 0) {
    return <p className="border-b border-[var(--line)] py-3 text-sm text-[var(--muted)]">No {label} yet.</p>;
  }
  return (
    <ul>
      {items.map((item, index) => {
        ...
        return (
          <li key={item} className="flex items-center gap-3 border-b border-[var(--line)] py-2">
            <span className="flex-1 truncate text-base text-[var(--text)]">{item}</span>
            <div className="flex flex-shrink-0 items-center gap-0.5">
              <div className="relative">
                {showHeartTip && <TutorialTooltip text="mark as preferred" onDismiss={() => onTutorialDismiss?.()} />}
                <button type="button" onClick={() => { onTogglePreferred(item); if (showHeartTip) onTutorialAdvance?.(); }} aria-pressed={isPreferred} aria-label={`${isPreferred ? "Unmark" : "Mark"} ${item} as preferred`}
              ...
              <div className="relative flex items-center gap-0.5">
                {showArrowTip && <TutorialTooltip text="set the order" onDismiss={() => onTutorialDismiss?.()} />}
                <button type="button" onClick={() => { onMoveUp(item); if (showArrowTip) onTutorialAdvance?.(); }} disabled={index === 0}
                  ...
                  aria-label={`Move ${item} up`}><ArrowUpIcon /></button>
                <button type="button" onClick={() => { onMoveDown(item); if (showArrowTip) onTutorialAdvance?.(); }} disabled={index === items.length - 1}
                  className={`rounded p-1.5 text-[var(--muted)] hover:text-[var(--text)] disabled:cursor-not-allowed disabled:opacity-40 ${showArrowTip ? "ring-2 ring-[var(--accent)] ring-offset-1 ring-offset-[var(--surface)]" : ""}`}
                  aria-label={`Move ${item} down`}><ArrowDownIcon /></button>
              </div>
              <button type="button" onClick={() => onRemove(item)} className="rounded p-1.5 text-[var(--muted)] hover:text-[var(--danger)]" aria-label={`Remove ${item}`}><TrashIcon /></button>
            </div>
          </li>
        );
      })}
...
function LanguageSelector({
  value, usedLangs, onChange, onValidityChange,
}: {
  value: string;
  usedLangs: string[];
  ...
  if (customMode) {
    ...
    return (
      <div className="flex flex-wrap items-center gap-1.5">
        <button
          type="button"
          onClick={() => { setCustomMode(false); setCheck({ status: "empty" }); onValidityChange(true); onChange(availableOptions[0]?.tag ?? DEFAULT_LANG); }}
        ...
        {check.status === "recognized" && (
          <span className="text-xs text-[var(--muted)]">→ {check.displayName}</span>
        )}
  ...
  return (
    <div className="flex items-center gap-1.5">
      <select
        value={value}
        onChange={(e) => {
          if (e.target.value === "__custom__") { setCustomMode(true); onValidityChange(true); return; }
...
function LanguageGroupCard({
  group, usedLangs, isOnly, tutorialStep,
  onChange, onRemove,
  onTutorialAdvance, onTutorialDismiss,
}: {
  group: GroupState;
  ...
  function addNames(raw: string) {
    ...
    onChange({ ...group, names: normalizeEntries([...group.names, ...additions]), nameInput: "" });
  }

  function addPronouns(raw: string) {
    const additions = splitInput(raw);
    if (additions.length === 0) return;
    onChange({ ...group, pronouns: normalizeEntries([...group.pronouns, ...additions]), pronounInput: "" });
  }
    ...
    <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface-strong)] p-5">
      ...
      <div className="mb-4 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <span className="text-xs font-medium uppercase tracking-wide text-[var(--muted)]">Language</span>
          <LanguageSelector
            value={group.lang}
            usedLangs={usedLangs}
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {/* Names */}
        <section>
          <h3 className="mb-1 text-base font-semibold text-[var(--text)]">Names</h3>
          {isEnglish && (
            <p className="mb-3 text-sm text-[var(--muted)]">Add one or more names — you can have as many as you like.</p>
          )}
          <div className="mb-3 flex gap-2">
            <input
              value={group.nameInput}
              onChange={(e) => onChange({ ...group, nameInput: e.target.value })}
              onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addNames(group.nameInput); } }}
            ...
            />
            <button type="button" onClick={() => addNames(group.nameInput)} className="min-h-11 rounded-lg bg-[var(--accent)] px-4 text-sm font-semibold text-[var(--accent-contrast)]">Add</button>
          </div>
          <EntryList
            label="names" items={group.names} preferred={group.preferredNames}
            tutorialStep={isEnglish ? tutorialStep : 0}
            onTutorialAdvance={onTutorialAdvance} onTutorialDismiss={onTutorialDismiss}
            onTogglePreferred={(item) => onChange({ ...group, preferredNames: group.preferredNames.includes(item) ? group.preferredNames.filter((e) => e !== item) : [...group.preferredNames, item] })}
            onMoveUp={(item) => { const i = group.names.indexOf(item); onChange({ ...group, names: moveEntry(group.names, i, i - 1) }); }}
            onMoveDown={(item) => { const i = group.names.indexOf(item); onChange({ ...group, names: moveEntry(group.names, i, i + 1) }); }}
            onRemove={(item) => onChange({ ...group, names: group.names.filter((e) => e !== item), preferredNames: group.preferredNames.filter((e) => e !== item) })}
          />
        </section>

        {/* Pronouns */}
        <section>
          <h3 className="mb-1 text-base font-semibold text-[var(--text)]">Pronouns</h3>
          {isEnglish && (
            <p className="mb-3 text-sm text-[var(--muted)]">Pick from common options or type your own.</p>
          )}
          {isEnglish && (
            <div className="mb-3 flex flex-wrap gap-2">
              {COMMON_PRONOUNS.map((entry) => (
                <button key={entry} type="button" onClick={() => addPronouns(entry)}
          ...
          <div className="mb-3 flex gap-2">
            <input
              ...
              onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addPronouns(group.pronounInput); } }}
              placeholder={isEnglish ? "they/them, she/her" : "Add pronoun…"}
              className="min-h-11 w-full rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-base text-[var(--text)]"
            />
            <button type="button" onClick={() => addPronouns(group.pronounInput)} className="min-h-11 rounded-lg bg-[var(--accent)] px-4 text-sm font-semibold text-[var(--accent-contrast)]">Add</button>
          </div>
          <EntryList
            label="pronouns" items={group.pronouns} preferred={group.preferredPronouns}
            onTogglePreferred={(item) => onChange({ ...group, preferredPronouns: group.preferredPronouns.includes(item) ? group.preferredPronouns.filter((e) => e !== item) : [...group.preferredPronouns, item] })}
            onMoveUp={(item) => { const i = group.pronouns.indexOf(item); onChange({ ...group, pronouns: moveEntry(group.pronouns, i, i - 1) }); }}
            onMoveDown={(item) => { const i = group.pronouns.indexOf(item); onChange({ ...group, pronouns: moveEntry(group.pronouns, i, i + 1) }); }}
            onRemove={(item) => onChange({ ...group, pronouns: group.pronouns.filter((e) => e !== item), preferredPronouns: group.preferredPronouns.filter((e) => e !== item) })}
          />
        </section>
      </div>
    </div>
...
function groupsToState(groups: LanguageGroup[]): GroupState[] {
  ...
  if (!states.some((g) => g.lang === DEFAULT_LANG)) {
    states.unshift({ lang: DEFAULT_LANG, names: [], preferredNames: [], pronouns: [], preferredPronouns: [], nameInput: "", pronounInput: "" });
  }
  return states;
}
...
export function ProfileEditor({
  ...
  const [groups, setGroups] = useState<GroupState[]>(() => groupsToState(initialGroups));
  ...
  function addLanguageGroup() {
    ...
    setGroups((prev) => [...prev, { lang: nextLang, names: [], preferredNames: [], pronouns: [], preferredPronouns: [], nameInput: "", pronounInput: "" }]);
    setSaved(false);
  }
  ...
  const hasUnusedLangs = COMMON_LANGUAGES.some((l) => !usedLangs.includes(l.tag)) || true;
  ...
  return (
    <div className="space-y-5">
      {isFirstTime && (
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface-strong)] p-4">
          <div className="flex items-start">
            ...
            <div className="mt-4 h-px flex-1" style={{ background: "var(--accent)" }} />
            <div className="flex shrink-0 flex-col items-center gap-1.5">
              <div className={`flex h-8 w-8 items-center justify-center rounded-full text-sm font-bold transition-colors ${saved ? "bg-[var(--accent)] text-[var(--accent-contrast)]" : "border-2 border-[var(--accent)] text-[var(--accent)]"}`}>
                {saved ? "✓" : "2"}
              </div>
              <span className="text-xs font-medium text-[var(--text)]">Add your info</span>
            </div>
            <div className="mt-4 h-px flex-1 transition-colors" style={{ background: saved ? "var(--accent)" : "var(--border)" }} />
            <div className="flex shrink-0 flex-col items-center gap-1.5">
              <div className={`flex h-8 w-8 items-center justify-center rounded-full border-2 text-sm font-bold transition-colors ${saved ? "border-[var(--accent)] text-[var(--accent)]" : "border-[var(--border)] text-[var(--muted)]"}`}>3</div>
              <span className={`text-xs transition-colors ${saved ? "font-medium text-[var(--text)]" : "text-[var(--muted)]"}`}>Share</span>
            </div>
          </div>
          {saved && profileHref && (
            <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-[var(--border)] pt-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-[var(--muted)]">Your profile is live and ready to share.</p>
                </div>
              </div>
              <a href={profileHref} className="inline-flex items-center gap-2 rounded-full bg-[var(--accent)] px-4 py-2 text-sm font-semibold text-[var(--accent-contrast)] transition-opacity hover:opacity-90">
                View your profile
      ...
      {error && <p className="border-b border-[var(--line)] py-2 text-sm text-[var(--danger)]">{error}</p>}
      {saved && !isFirstTime && <p className="border-b border-[var(--line)] py-2 text-sm text-[var(--success)]">Profile updated.</p>}
