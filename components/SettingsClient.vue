...
onMounted(() => {
  watchEffect(async () => {
    ...
    try {
      const [actorData, profileData] = await Promise.all([
        fetchActorProfile(d),
        fetchProfileRecords(d),
      ]);
...
const isFirstTime = computed(
  () =>
    ...
    groups.value.every((g) => g.names.length === 0 && g.pronouns.length === 0),
);
</script>
...
<template>
  ...
  <main
    v-if="did === null || actor === undefined || groups === null"
    class="mx-auto w-full max-w-6xl px-4 pb-8 sm:px-6"
  >
    <section
      class="space-y-4 rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-6"
    >
      <div
        class="h-8 w-48 animate-pulse rounded-md bg-[var(--surface-strong)]"
      />
      <div
        class="h-4 w-64 animate-pulse rounded-md bg-[var(--surface-strong)]"
      />
  ...
  <main v-else class="mx-auto w-full max-w-6xl px-4 pb-8 sm:px-6">
    ...
    >
      ...
      <ProfileEditor
        ...
        :profile-href="profileHref ?? undefined"
      />
    </section>
  </main>
</template>
