...
onMounted(() => {
  ...
  const observer = new IntersectionObserver(async (entries) => {
    ...
    loading.value = true;
    try {
      const params = new URLSearchParams({ actor: props.did, cursor: cursor.value, limit: String(PAGE_LIMIT) });
      const res = await fetch(`https://public.api.bsky.app/xrpc/app.bsky.graph.getFollows?${params}`);
      if (!res.ok) { retryAfter.value = Date.now() + 3000; return; }
      const data = await res.json() as { follows: { did: string; handle: string; displayName?: string; avatar?: string }[]; cursor?: string };
      follows.value = [...follows.value, ...data.follows.map(f => ({ did: f.did, handle: f.handle, displayName: f.displayName ?? null, avatar: f.avatar ?? null }))];
      cursor.value = data.cursor;
      hasMore.value = Boolean(data.cursor);
    } catch { retryAfter.value = Date.now() + 3000; }
    ...
    <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
      ...
      <template v-if="loading">
        <div v-for="i in SKELETON_COUNT" :key="i" class="flex flex-col items-center gap-3 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4">
          ...
          <div class="w-full space-y-1.5"><div class="h-3 animate-pulse rounded bg-[var(--surface-strong)]" /><div class="mx-auto h-3 w-3/4 animate-pulse rounded bg-[var(--surface-strong)]" /></div>
        </div>
      </template>
    </div>
    <div v-if="hasMore" ref="sentinelRef" class="mt-8 h-px" />
    <p v-if="!hasMore && follows.length > 0" class="mt-8 text-center text-xs text-[var(--muted)]">All caught up ✓</p>
  </div>
</template>
