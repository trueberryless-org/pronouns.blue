...
const activeGroups = computed(() =>
  props.groups.filter((g) => g.names.length > 0 || g.pronouns.length > 0),
);
const hasAny = computed(() => activeGroups.value.length > 0);
const hasBskyPronouns = computed(
  () => !hasAny.value && !!props.bskyFallbackPronouns,
);
...
<template>
  <main
    class="mx-auto w-full max-w-3xl flex-1 min-h-screen px-4 pt-12 pb-16 sm:pt-16"
  >
    <section class="w-full py-4">
      ...
      <div class="mb-8 flex flex-col items-center text-center">
        ...
        <div
          v-if="avatar"
          class="h-24 w-24 rounded-full border border-[var(--border)] bg-cover bg-center"
        ...
        <div
          v-else
          class="flex h-24 w-24 items-center justify-center rounded-full border border-[var(--border)] bg-[var(--tag-bg)] text-3xl font-semibold text-[var(--text)]"
        >
        ...
        <p class="mt-1 flex items-center gap-2 text-base text-[var(--muted)]">
          ...
          <a
            ...
            class="translate-y-0.3 text-[var(--muted)] transition-colors hover:text-[var(--accent)]"
          >
            <svg
              role="img"
              viewBox="0 0 24 24"
      ...
      <div v-if="hasAny" class="space-y-8">
        <div v-for="group in activeGroups" :key="group.lang" class="space-y-4">
          ...
          <div v-if="showLangLabels" class="flex items-center gap-2">
            <span
              class="rounded-full border border-[var(--border)] px-2.5 py-0.5 text-xs font-medium text-[var(--muted)]"
              >{{ langLabel(group.lang) }}</span
            >
          ...
          <div
            :class="[
              'grid gap-6',
              group.names.length > 0 && group.pronouns.length > 0
                ? 'md:grid-cols-2'
                : '',
            ]"
          >
            ...
            <div v-if="group.names.length > 0">
              <div class="mb-3 flex items-center gap-2">
                <h2
                  class="text-sm font-semibold uppercase tracking-wide text-[var(--muted)]"
                >
                  Names
                </h2>
              </div>
              <ul>
                <li
                  ...
                  :key="item"
                  class="flex items-center justify-between border-b border-[var(--line)] py-2 text-base text-[var(--text)]"
                >
                  <span>{{ item }}</span>
                  <span
                    v-if="group.preferredNames.includes(item)"
                    title="Preferred"
                  >
                    <HeartIcon
                      :filled="true"
                      class="h-5 w-5 text-[var(--danger)]"
                    />
                  </span>
                </li>
              </ul>
            ...
            <div v-if="group.pronouns.length > 0">
              ...
              <ul>
                <li
                  v-for="item in group.pronouns"
                  :key="item"
                  class="flex items-center justify-between border-b border-[var(--line)] py-2 text-base text-[var(--text)]"
                >
                  <span>{{ item }}</span>
                  <span
                    v-if="group.preferredPronouns.includes(item)"
                    title="Preferred"
                  >
                    <HeartIcon
                      :filled="true"
      ...
      <div v-else-if="hasBskyPronouns" class="grid gap-6">
        <div>
          <div class="mb-3 flex items-center gap-2">
            ...
            <span
              class="flex items-center gap-1 rounded-full border border-[var(--border)] px-1.5 py-0.5 text-[10px] text-[var(--muted)]"
            >
              <svg
                role="img"
                viewBox="0 0 24 24"
      ...
      <!-- Empty state -->
      <p v-else class="text-center text-[var(--muted)]">
