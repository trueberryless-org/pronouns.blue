...
onMounted(() => {
  watchEffect(async () => {
    ...
    try {
      const r = await fetch(
        `https://public.api.bsky.app/xrpc/app.bsky.actor.getProfile?actor=${d}`,
      );
...
const label = computed(
  () => user.value?.displayName ?? user.value?.handle ?? user.value?.did ?? "U",
);
</script>
...
<template>
  <section v-if="user" class="grid gap-4 md:grid-cols-2">
    <NuxtLink
      to="/settings"
      class="rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-6 transition hover:border-[var(--accent)]"
    >
    ...
    >
      <div class="flex items-center gap-3">
        <span
          v-if="user.avatar"
          class="h-12 w-12 rounded-full border border-[var(--border)] bg-cover bg-center"
        ...
        <span
          v-else
          class="flex h-12 w-12 items-center justify-center rounded-full border border-[var(--border)] bg-[var(--surface-strong)] text-lg font-semibold text-[var(--text)]"
