...
watch(normalizedQuery, (q) => {
  ...
  searchTimeout = setTimeout(async () => {
    ...
    try {
      ...
      suggestions.value = (data.actors ?? []).map((a) => ({
        did: a.did,
        handle: a.handle,
...
<template>
  ...
  >
    ...
    <kbd
      class="hidden items-center gap-0.5 rounded border border-[var(--border)] px-1.5 py-0.5 font-mono text-[10px] leading-none lg:inline-flex"
  ...
  <Teleport to="body">
    ...
    >
      <div
        class="mx-4 w-full max-w-lg overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--surface)] shadow-2xl"
      >
        <div
          class="flex items-center gap-3 border-b border-[var(--border)] px-4 py-3"
        >
          <svg
            viewBox="0 0 24 24"
            class="h-5 w-5"
        ...
        <div
          v-if="loading || suggestions.length > 0 || normalizedQuery"
          class="max-h-72 overflow-auto p-2"
        >
          <p v-if="loading" class="px-3 py-2 text-sm text-[var(--muted)]">
            Searching…
          </p>
          <p
