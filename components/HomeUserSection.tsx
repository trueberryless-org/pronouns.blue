...
/**
 * Client component that renders personalised home-page sections for signed-in
 * users: the first-time welcome banner (if the profile is empty) and the
 * settings/profile shortcut cards.
 *
 * By keeping this client-only, the home page itself can be statically rendered
 * and served from the CDN without ever invoking a serverless function.
 */
export function HomeUserSection() {
  const [data, setData] = useState<MeResponse | null>(null);

  useEffect(() => {
    // Only fetch if the user appears to be signed in (middleware sets did-public)
        ...
        <div className="relative space-y-6">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--muted)]">
              You&apos;re signed in — now add your names and pronouns so others
              can find you.
            </p>
          </div>
