...
const { data, error } = await useAsyncData(`profile-${handle}`, async () => {
  ...
  return {
    ...
    initialFollows: followsRes.follows.map(f => ({ did: f.did, handle: f.handle, displayName: f.displayName ?? null, avatar: f.avatar ?? null })),
...
useHead({
  ...
  meta: [
    { name: 'description', content: `View @${actor.handle}'s preferred names and pronouns on pronouns.blue` },
    { property: 'og:title', content: `@${actor.handle} – pronouns.blue` },
    { property: 'og:description', content: `View @${actor.handle}'s preferred names and pronouns on pronouns.blue` },
...
<template>
  <div>
    ...
    <section v-if="initialFollows.length > 0" class="mx-auto w-full max-w-3xl px-4 pb-16 sm:px-6">
      <div class="border-t border-[var(--border)] pt-10">
        ...
        <FollowsGrid :initial-follows="initialFollows" :initial-cursor="initialCursor" :did="actor.did" />
      </div>
    </section>
